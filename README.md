...
本リリース（追加実装）で作る機能
- **LINE通知機能 (Messaging API):** 保証期限等のリマインドをLINEで受信。※初期はシンプルなテキスト通知を実装。
...
| **Auth** | **OmniAuth LINE** | **LINEログイン**によるクイックなユーザー登録と、生活動線への密着 |
| **Notification** | **LINE Messaging API** | メールの埋没を防ぎ、開封率の高いLINEでのリマインド通知を実現 |
...
4. **LINEログイン & Messaging APIの採用理由:** ターゲット層（20-40代）が最も日常的に利用するLINEを基盤とすることで、ログインの心理的ハードルを下げ、通知の開封率を最大化します。メールに比べ情報の埋没を防ぎやすく、「保管庫＋通知」のサイクルをLINEという一つのアプリ内で完結させることで、利便性を提供します。また、Webhookを用いた双方向の連携は技術的なチャレンジ要素としても選定しました。
...
### 本サービスの概要（700文字以内）
## サービス概要
「Pake-Log」は、家族のための「デジタル説明書保管庫」です。捨てづらいパッケージ裏や簡易説明書を撮影し、思い出の写真とは別に管理することで、必要な情報へ1秒でアクセスできる体験を提供します。

## 想定ユーザー
ライフステージの変化で管理すべき物が増える20-40代の子育て・共働き世代。探し物の時間を減らし、暮らしを身軽にしたい方を想定しています。

## 解決したい課題
「撮った写真が思い出に埋もれて見つからない」という不快感と、物理的な保管場所の圧迫を解消します。情報の定位置を確立し、脳のメモリを使わない安心感を提供します。

## 主な機能 (MVP)
- **LINEログイン**: 登録の摩擦を最小化し、生活動線に密着。
- **アイテム投稿・検索**: 画像最大3枚、カテゴリ検索、キーワード曖昧検索。
- **画像最適化**: Cloudinaryによる自動圧縮で、可読性と軽量化を両立。
- **拡大ビューワー**: ピンチ操作で細かい文字までストレスなく確認可能。

## 差別化ポイント
既存サービスが公式PDFに強いのに対し、地域の時刻表やレシピ等「ネットにない自分だけのアナログ情報」の保管に特化しています。スマホのアルバムを純粋な「思い出の場所」として守り抜く専用保管庫です。
